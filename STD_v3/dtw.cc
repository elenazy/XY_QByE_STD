...
// this also require that each test frame match with only one frame in the temlate.
...
//this dtw use different local constrain compared with previour version
// (i-2,j-1)
// (i-1,j-1)
// (i,j-1)
float SLN_DTW_c2(const infra::matrix &dist, infra::vector &area) {
    ...
    // initialize
    // trace(i,j) = 0 denote the precedent point of (i,j) is (i-2,j-1)
    // trace(i,j) = 1                                        (i-1,j-1)
    // trace(i,j) = 2                                        (i,j-1)
    int i, j;
    double cost_0, cost_1, cost_2;
    double avg_cost_0, avg_cost_1, avg_cost_2;
    ...
    for (i = 1; i < height; i++) {
        for (j = 1; j < width; j++) {
            ...
            if (i < 2) {
                cost_0 = BIG_FLT*2;
                avg_cost_0 = BIG_FLT*2;
            } else {
                cost_0 = dist(i, j) + cost(i-2, j-1);
                avg_cost_0 = cost_0 / (1 + length(i-2, j-1));
            }
            cost_1 = dist(i, j) + cost(i-1, j-1);
            cost_2 = dist(i, j) + cost(i, j-1);
            avg_cost_1 = cost_1 / (1 + length(i-1, j-1));
            avg_cost_2 = cost_2 / (1 + length(i, j-1));

            // choose the one which lead to the minimum cost as the precedent point
            if (avg_cost_0 <= avg_cost_1 && avg_cost_0 <= avg_cost_2) {
                avg_cost(i, j) = avg_cost_0;
                cost(i, j) = cost_0;
                length(i, j) = 1 + length(i-2, j-1);
                trace(i, j) = trace(i-2, j-1);
            } else if (avg_cost_1 <= avg_cost_0 && avg_cost_1 <= avg_cost_2) {
                avg_cost(i, j) = avg_cost_1;
                cost(i, j) = cost_1;
                length(i, j) = 1 + length(i-1, j-1);
                trace(i, j) = trace(i-1, j-1);
            } else if (avg_cost_2 <= avg_cost_1 && avg_cost_2 <= avg_cost_0){
                avg_cost(i, j) = avg_cost_2;
                cost(i, j) = cost_2;
                length(i, j) = 1 + length(i, j-1);
                trace(i, j) = trace(i, j-1);
            } else {
                std::cout << "wrong selection at SLN DTW" << std::endl;
            }
        }
    }
    int end_point = avg_cost.row(height-1).argmin();
