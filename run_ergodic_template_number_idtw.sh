...
do
    for tempalte_num in 1 2 4 8 12 16 24 32;
    do
        for random_num in `seq 5`;
        do
            keyword_dir="$feat_dir/at_${keyword_type}_${tempalte_num}_${random_num}/"
            ...
            if [ $stage -le 0 ]; then
                ...
                do
                    ...
                    {
                        #echo "./STD_v3/i_dtw_std $keyword_dir ./tmp/${keyword_list_basename}${i} $test_dir $test_list_file $fea_type $distance_type $do_mvn $result_dir"
                              ./STD_v3/i_dtw_std_phrase $keyword_dir ./tmp/${keyword_list_basename}${i} $test_dir $test_list_file $fea_type $distance_type $do_mvn $result_dir
                    } &
                    done
                    wait
                done
